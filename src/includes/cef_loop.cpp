...
uv_thread_t CefLoop::tid;
Settings *CefLoop::options_;

bool CefLoop::initialized_ = false;
bool CefLoop::running_ = false;

void CefLoop::Init(Settings* options) {
  options_ = options;
  //uv_thread_create(&tid, CefLoop::Init,NULL);
  Init((void*)NULL);
}

void CefLoop::Init(void* param) {
