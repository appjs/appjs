module.exports = function(){
  function Bitfield(values, source, prop){
    return new BitfieldHandler(values, source, prop);
  }

  function BitfieldHandler(values, source, prop){
    this.values = values;
    this.source = source;
    this.prop = prop;
    this.list = Object.keys(values).sort();
    return Proxy.create(this, Bitfield.prototype);
  }
  ...
  function get(key){
    return (this.values[key] & this.source[this.prop]) > 0;
  }

  function set(key, value){
    value ? (this.source[this.prop] |= this.values[key]) : (this.source[this.prop] &= ~this.values[key]);
  }
  ...
  return function stylesForWindow(window){
    if (typeof Proxy === 'undefined') return;
    window.styles = new Bitfield(Bitfield.styles, window, 'style');
    window.exStyles = new Bitfield(Bitfield.exStyles, window, 'extendedStyle');
  };
}();
