...
function Window(nativeWindow){
  ...
  nativeWindow.on('ready', function(){
    ...
    setTimeout(function(){
      ...
      window.frame = frame;
      window.Function('log', 'browserRequire', 'window.require = '+function require(request){
        var ret;
        browserRequire(window, request, function(e, result){
          if (e) log(request, 'require', 'browser', e);
          ret = e || result;
        });
        return ret;
      })(log, browserRequire);
