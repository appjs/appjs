...
function Window(win){
  ...
  win.imbue = function imbue(){
    ...
    window.console.open = function open(){ win.closeDevTools() };
    window.emit('ready');
  };
...
var screen = {
  ...
  width: App.prototype.screenHeight
};

var FrameImpl = new RefType(['left', 'top', 'width', 'height', 'title'], {
  get right(){
...
Frame.prototype = FrameImpl.prototype = {
  ...
  center: function center(){
    frames.get(this).setPosition((screen.height() - this.height) / 2, (screen.width() - this.width) / 2);
    return this;
  },
  ...
  move: function move(top, left, width, height){
    frames.get(this).setPosition(top, left, width, height);
    return this;
  },
  resize: function resize(width, height){
    frames.get(this).setSize(width, height)
    return this
  }
};
