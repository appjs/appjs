...
var installMethods = function(){
  var windowMethods = [
    ...
    'restore', 'drag', 'drop', 'show', 'hide', 'setPosition', 'setSize'
  ].map(function(key){
...
function RefType(properties, accessors){
  ...
  var Ctor = function(ref){
    return Proxy.create(new RefHandler(ref, this), Ctor.prototype);
  };
  return Ctor;
}
...
var Frame = new RefType(['left', 'top', 'width', 'height', 'title'], {
  get right(){
    return App.prototype.screenWidth() - this.left - this.width;
  },
  set right(v){
    this.width = Math.max(0, App.prototype.screenWidth() - this.left - v);
  },
  get bottom(){
    return App.prototype.screenHeight() - this.top - this.height;
  },
  set bottom(v){
    this.height = Math.max(0, App.prototype.screenHeight() - this.top - v);
  }
});


Frame.prototype.center = function center(){
  this.top = (App.prototype.screenHeight() - this.height) / 2;
  this.left = (App.prototype.screenWidth() - this.width) / 2
};
